...
- `:plugins`: Contains various plugin sub-projects (e.g., `:plugins:Osmand-Nautical`, `:plugins:Osmand-Skimaps`, and APRS core sources under `plugins/aprs-core/` compiled into `:OsmAnd`).
...
- **APRS core plugin:** `OsmAnd/plugins/aprs-core/` (merged into `:OsmAnd` via source sets).
